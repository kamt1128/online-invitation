import { defineConfig } from "vite";
import react from "@vitejs/plugin-react";
import { VitePWA } from "vite-plugin-pwa";

export default defineConfig({
  //base: "/online-invitation/",
  plugins: [
    react(),
    VitePWA({
      registerType: "autoUpdate",
      strategies: "generateSW",
      injectRegister: "auto",

      manifest: {
        name: "Invitación digital",
        short_name: "Invita Digital",
        description: "Invitación digital a eventos formales",
        start_url: "/online-invitation/#/admin/",
        scope: "/online-invitation/#/admin/",
        display: "standalone",
        background_color: "#8a0cd2",
        theme_color: "#8a0cd2",
        orientation: "portrait",
        icons: [
          {
            src: "/online-invitation/icons/icon-192.png",
            sizes: "192x192",
            type: "image/png",
          },
          {
            src: "/online-invitation/icons/icon-512.png",
            sizes: "512x512",
            type: "image/png",
          },
        ],
      },

      workbox: {
        cleanupOutdatedCaches: true,
        clientsClaim: true,
        skipWaiting: true,
        navigateFallback: "/online-invitation/#/index.html",
      },
    }),
  ],
});
